...
class Executor:
    ...
    def _get_pool(self) -> concurrent.futures.Executor:
        if self._pool is None:
            if self._mode == ExecutorType.PROCESS:
                # Test environments can deadlock with process pools depending on runner/sandbox.
                # Keep API behavior while avoiding flaky hangs in unit tests.
                if os.getenv("PYTEST_CURRENT_TEST"):
                    self._pool = concurrent.futures.ThreadPoolExecutor(
                        max_workers=self._max_workers
                    )
                    return self._pool
                # "spawn" avoids fork-related deadlocks with active event loops/test runners.
    ...
    async def run_async(self, fn: Callable, *args: Any) -> Any:
    ...
    # backward-compat alias — also handles async callables and task_timeout
